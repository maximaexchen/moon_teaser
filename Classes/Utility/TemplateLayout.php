...
class TemplateLayout implements SingletonInterface
{
    ...
    public function getAvailableTemplateLayouts($pageUid)
    {
        ...
        if (isset($GLOBALS['TYPO3_CONF_VARS']['EXT']['tx_moonteaser']['templateLayouts'])
            && is_array($GLOBALS['TYPO3_CONF_VARS']['EXT']['tx_moonteaser']['templateLayouts'])
        ) {
            $templateLayouts = $GLOBALS['TYPO3_CONF_VARS']['EXT']['tx_moonteaser']['templateLayouts'];
        }
    ...
    protected function getTemplateLayoutsFromTsConfig($pageUid)
    {
        ...
        if (isset($pagesTsConfig['tx_moonteaser.']['templateLayouts.']) && is_array($pagesTsConfig['tx_moonteaser.']['templateLayouts.'])) {
            $templateLayouts = $pagesTsConfig['tx_moonteaser.']['templateLayouts.'];
        }
        return $templateLayouts;
    }
}
